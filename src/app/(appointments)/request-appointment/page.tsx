...
const AppointmentForm: React.FC = () => {
  ...
  const times = Array.from({ length: 13 }, (_, i) => `${8 + i}:00 ${i < 4 ? 'AM' : 'PM'}`);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const adjustedHours = meridian === 'PM' && hours !== 12 ? hours + 12 : hours;
      const dateTime = new Date(year, month - 1, day, adjustedHours, minutes);
      ...
      const appointmentDetails = {
        ...
        patientId: patientId, // Replace with actual patient ID from authentication
      ...
      // API call to submit the appointment
