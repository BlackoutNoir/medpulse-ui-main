...
import { addContactValidator } from '@/lib/validations/add-contact'
...
export async function POST(req: Request) {
  try {
    ...
    const isAlreadyAdded = (await fetchRedis(
      'sismember',
      `user:${idToAdd}:incoming_contact_requests`,
      session.user.id
    )) as 0 | 1
    ...
    await db.sadd(`user:${idToAdd}:incoming_contact_requests`, session.user.id)
